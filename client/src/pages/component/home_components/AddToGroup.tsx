...
function AddToGroup({setFocused, focused, setExtend, extend, addFriend, friend}: Prop) {
    ...
    return <>
        <div>
            <div className={["w-[clamp(13rem,25vw,100rem)] h-[5vh] flex flex-col rounded-[1vw]", extend ? "h-[25vh] overflow-y-scroll no-scrollbar" : "overflow-y-visible", focused ? "bg-[#F5ECD7] shadow-[0_4px_25px_0_rgba(0,0,0,0.5)]": ""].join(" ")}>
            <input ref={input} type="text" onClick={() => setErrorMessage("")} onFocus={() => {setFocused(true); setErrorMessage("")}} onKeyDown={(e) => {setErrorMessage(""); handleKeyDown(e, e.currentTarget.value)}} value={friendName} placeholder='Add Friend' onChange={(e) => {setfriendName(e.currentTarget.value); handleType(e.currentTarget.value)}} className={["focus:outline-none rounded-full h-[5vh] w-full flex-none text-center text-black", focused ? "font-bold" : ""].join(" ")}/>
