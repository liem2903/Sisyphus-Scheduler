...
function AddToGroup({setFocused, focused, setExtend, extend, addFriend, friend}: Prop) {
    ...
    return <>
        <div>
            <div className={["w-[clamp(13rem,25vw,100rem)] h-[5vh] overflow-y-scroll no-scrollbar flex flex-col rounded-[1vw] ", extend ? "h-[25vh]" : "", focused ? "bg-white": ""].join(" ")}>
                <input ref={input} type="text" onClick={() => setErrorMessage("")} onFocus={() => {setFocused(true); setErrorMessage("")}} onKeyDown={(e) => {setErrorMessage(""); handleKeyDown(e, e.currentTarget.value)}} value={friendName} placeholder='Add Friend' onChange={(e) => {setfriendName(e.currentTarget.value); handleType(e.currentTarget.value)}} className={["focus:outline-none rounded-full h-[5vh] w-full flex-none text-center text-black", focused ? "bg-white font-bold" : ""].join(" ")}/>
